...
for i=1:num_of_samples
    [cell4(i).neighlist]=neighlist_creation(cell4(i).mask_cell,number_of_pixels);
end
...
if indx ==1
    Interaction_heatmap(hfig1,new_cooc{1}-new_cooc{2},accumarray(horzcat(cell4(:).clusters)',1),handles, vertcat(clusteri{:}));
else
    indx=indx-1;
    ...
    Interaction_heatmap(hfig1,tt,accumarray(horzcat(cell4(:).clusters)',1),handles, vertcat(clusteri{:}));
end
...
set(h,'windowbuttonmotionfcn',{@mousemove_interaction,handles});